...
export default async function BlogPostPage({ params }) {
  ...
  return (
    <article className="max-w-4xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
      ...
      <Link
        href="/blog"
        className="inline-flex items-center gap-1.5 text-sm text-surface-400 hover:text-surface-600 
      ...
      <div className="flex items-center gap-4 mb-8 pb-8 border-b border-surface-100">
        ...
        <Link href={`/author/${post.author}`} className="flex-shrink-0">
          <div className="w-11 h-11 rounded-full overflow-hidden bg-gradient-to-br from-primary-100 to-primary-200 
      ...
      <div className="mt-16 pt-8 border-t border-surface-100">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Link href={`/author/${post.author}`} className="flex-shrink-0">
              <div className="w-10 h-10 rounded-full overflow-hidden bg-gradient-to-br from-primary-100 to-primary-200">
                {avatarSrc ? (
                  <img src={avatarSrc} alt={author?.name || post.author} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center">
          ...
          <Link
            href="/blog"
            className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium bg-surface-50 
                       text-surface-700 rounded-xl border border-surface-200 hover:bg-surface-100 transition-colors"
          >
            More posts
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
